...
def list_items(
    ...
    try:
        ...
        params["select[]"] = list(required)[:50]  # Limit to 50 fields to avoid URL length issues

        rows = bx.fetch_all(params, entity)

        # Flatten deal contacts (PHONE/EMAIL/NAME) - with error handling
        if entity == "deal":
            for row in rows:
                try:
                    ...
                    if cid:
                        try:
                            ...
                            # Log contact fetch error but don't fail the entire request
                            print(f"Warning: Could not fetch contact {cid}: {str(contact_err)}")
                            pass
                except Exception as row_err:
                    # Log row processing error but don't fail
                    print(f"Warning: Could not process row: {str(row_err)}")
                    pass
