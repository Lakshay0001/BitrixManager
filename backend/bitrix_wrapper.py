...
def ensure_slash(u: str) -> str:
    u = u.strip().strip('"').strip("'")  # remove quotes
    if not u.endswith("/"):
        u += "/"
    return u
...
class BitrixWrapper:
    ...
    def fetch_field_definitions(self, entity: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        for code, info in fields.items():
            ...
            if isinstance(info, dict):
                label = (
                    info.get("listLabel")
                    or info.get("formLabel")
                    or info.get("filterLabel")
                    or info.get("title")
                    or ""
                ).strip()
    ...
    # ADD THIS (do not modify existing functions)

    def fetch_all_users_full(self):
        """
        Fetch all users with full profile info from Bitrix.
        Safe: does NOT affect fetch_all_users()
        """
        out = []
        p = {"start": 0}
    ...
    def fetch_departments(self) -> dict:
        """Fetch all departments and return {id: name}"""
        out = {}
        p = {"start": 0}
        while True:
            ok, resp = self._call("department.get", p, "get")
            if not ok or "result" not in resp:
                break
            for dept in resp["result"]:
                out[str(dept["ID"])] = dept.get("NAME", "")
    ...
    def delete_single(self, item_id, entity):
        ...
        if str(item_id).startswith("UF_"):
            # Proper userfield deletion based on entity
            ok, resp = self._call(
                f"crm.{entity}.userfield.delete", {"id": item_id}, "post"
            )
        else:
    ...
    def fetch_all(self, params: Dict[str, Any], entity: str) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            for row in current_batch:
                row["PHONE_VALUE"] = (
                    row.get("PHONE", [{}])[0].get("VALUE", "")
                    if row.get("PHONE")
                    else ""
                )
                row["PHONE_TYPE"] = (
                    row.get("PHONE", [{}])[0].get("VALUE_TYPE", "")
                    if row.get("PHONE")
                    else ""
                )
                row["EMAIL_VALUE"] = (
                    row.get("EMAIL", [{}])[0].get("VALUE", "")
                    if row.get("EMAIL")
                    else ""
                )
                row["EMAIL_TYPE"] = (
                    row.get("EMAIL", [{}])[0].get("VALUE_TYPE", "")
                    if row.get("EMAIL")
                    else ""
                )
            ...
            for row in current_batch:
                try:
                    processed.append(
                        self.user_resolver.resolve_user_fields_in_record(row)
                    )
                except Exception:
    ...
    def update_single(
        self, idv: str, entity: str, fields: Dict[str, Any]
    ) -> Dict[str, Any]:
        ok, resp = self._call(
            f"crm.{entity}.update", {"id": idv, "fields": fields}, "post"
        )
        return resp if ok else {"error": "network"}
